...
export default {
  ...
  mounted() {
    if (this.isTeacher) {
      ...
      let xx = BJY.ApplyList.create({
        element: $(".bjy-apply-list .placeholder"), // 组件父容器
        replace: false, // 是否替换父容器下的 DOM
      });
      // setTimeout(() => {
      //   console.log('BJY.UserApply', BJY.UserApply)
      //   console.log('BJY.UserssssssssssssApply', this.$ref.xx)
      //   BJY.UserApply.methods.acceptApply()
      //   console.log('xxxxxxxxxxxxxxxxxxxx', xx);
      //   xx.accept()
      // }, 2000);
    }
    eventEmitter.trigger(eventEmitter.UPDATE_USER_LAYER_TRIGGER, {user: store.get("user") })
